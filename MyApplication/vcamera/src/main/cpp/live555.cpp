...
int live555_stop()
{
 ...
 {
     if(g_pQueue) {
         return g_pQueue->releasebuffer(index);
     }else{
         return 0;
     };
 }
...
void openURL(UsageEnvironment& env, char const* progName, char const* rtspURL) {
    ...
    if (rtspClient == NULL) {
        ...
        TRACK("Failed to create a RTSP client for URL:%s\%s",rtspURL,env.getResultMsg());
        return;
    }
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString) {
    do {
        ...
        if (resultCode != 0) {
            ...
            TRACK(" Failed to get a SDP description: %s",resultString);
            delete[] resultString;
            break;
        }
        ...
        TRACK("Got a SDP description: %s",sdpDescription);
...
void DummySink::afterGettingFrame(unsigned frameSize, unsigned numTruncatedBytes,
    ...
    if(g_pQueue)
    {
        ...
        if(type != 9 )
        {
            TRACK("zachery afterGettingFrame,g_pQueue->push");
//            Nalu * pNalu = NULL;
//std::shared_ptr<>
